...
struct ScannerAST
{
    ...
    ScannerAST(string nombreArchivo_)
    {
...
vector<pair<string, string>> ScannerAST::GetTokens()
{
...
char ScannerAST::peekchar()
{
    char c = lineas[fila][columna];
    return c;
}

bool ScannerAST::verificarIndentacion(int n)
{
...
void ScannerAST::imprimirTokens()
{
...
void ScannerAST::imprimirErrores()
{
    cout << "------------------------------------------" << endl;
    vector<Token>::iterator it;
    cout << errores.size();
...
{
    ScannerAST scan("E:/semestre_2023-01/Compiladores/Compilador Version Final/Compilador Version Final/archivo4.txt");
    scan.leerArchivo();
    scan.escaner();
    scan.imprimirTokens();
